...
export default function AIAssistPanel({ pageTitle, blocks, onAddBlocks }: Props) {
  ...
  return (
    <div className="bg-gradient-to-b from-violet-50 to-white border border-violet-200 rounded-xl p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-semibold text-violet-900">AI Assist</h3>
      </div>

      {/* Mode Toggle */}
      <div className="flex gap-1 mb-3 bg-violet-100 rounded-lg p-0.5">
        <button
          onClick={() => setMode("generate")}
          className={`flex-1 text-xs py-1.5 rounded-md transition-colors ${
            mode === "generate" ? "bg-white text-violet-900 shadow-sm" : "text-violet-600"
          }`}
        >
          Block
        </button>
        <button
          ...
          className={`flex-1 text-xs py-1.5 rounded-md transition-colors ${
            mode === "page" ? "bg-white text-violet-900 shadow-sm" : "text-violet-600"
          }`}
        >
          Full Page
        </button>
      ...
      <textarea
        ...
        rows={mode === "page" ? 4 : 2}
        className="w-full px-3 py-2 text-xs border border-violet-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-400 resize-none mb-3"
      />
      ...
      {mode === "generate" ? (
        <>
          ...
          <div className="grid grid-cols-2 gap-1.5 mb-3">
            {quickBlocks.map((qb) => (
              <button
                ...
                disabled={loading}
                className="text-xs px-2 py-1.5 bg-white border border-violet-200 rounded-md hover:bg-violet-50 text-violet-700 transition-colors disabled:opacity-50"
              >
                {qb.label}
              </button>
            ))}
          ...
          <button
            ...
            disabled={loading}
            className="w-full text-xs px-3 py-1.5 bg-white border border-violet-200 rounded-md hover:bg-violet-50 text-violet-700 transition-colors disabled:opacity-50"
          >
        ...
        <button
          ...
          className="w-full text-xs px-3 py-2 bg-violet-600 text-white rounded-lg hover:bg-violet-700 transition-colors disabled:opacity-50 font-medium"
        >
      ...
      {/* Result */}
      {lastResult && !loading && (
        <div className="mt-3 p-2 bg-white rounded-lg border border-violet-100">
