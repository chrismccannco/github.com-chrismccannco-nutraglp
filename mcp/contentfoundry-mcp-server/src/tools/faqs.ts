...
export function registerFaqTools(server: McpServer) {
  server.registerTool(
    "cf_list_faqs",
    {
      title: "List FAQs",
      description:
        "List published FAQs from ContentFoundry, optionally filtered by category. Useful for answering customer questions, training chatbots, or auditing content coverage.",
      inputSchema: {
        category: z
          .string()
          .optional()
